...
After installing the required dependencies (see MP-SPDZ doc) clone this repository and enter the following commands:

```
make -j8 tldr
make mascot-ecdsa-party.x
```

Then on two different terminals, run:

```
./mascot-ecdsa-party.x -p 0 -h localhost
./mascot-ecdsa-party.x -p 1 -h localhost
```
----------------------
